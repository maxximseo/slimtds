...
final class StatsRepository
{
     ...
     *
     * @return list<array{offer_name:string, conversions:int, approved:int, all_clicks:int, all_epc:float, search_conversions:int, search_clicks:int, search_epc:float}>
     */
    public function digestOfferEpc(string $sinceIso): array
    {
        ...
        $rows = $this->db->fetchAll(
            "WITH lead_offers AS (
             ...
             SELECT COALESCE(o.name, 'Offer unknown') AS offer_name,
                    lo.conversions,
                    lo.approved,
                    COALESCE(ac.clicks, 0)::int AS all_clicks,
                    CASE WHEN COALESCE(ac.clicks, 0) > 0
                         THEN round(lo.payout / ac.clicks, 4)
                         ELSE 0
                    END::float AS all_epc,
             ...
             LEFT JOIN all_clicks ac ON ac.offer_id IS NOT DISTINCT FROM lo.offer_id
        ...
        return array_map(static fn ($row) => [
            ...
            'conversions'       => (int)$row['conversions'],
            'approved'          => (int)$row['approved'],
            'all_clicks'        => (int)$row['all_clicks'],
            'all_epc'           => (float)$row['all_epc'],
